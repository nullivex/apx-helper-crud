...
var Helper = function(Model,name,description,options){
  ...
  this.options = new ObjectManage(this.options)
...
Helper.prototype.find = function(apx,req,res,next){
  if(apx.roles && this.options.get('useRoles') && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.find')){
...
Helper.prototype.save = function(apx,req,res,done){
  if(apx.roles && this.options.get('useRoles') && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.save')){
    res.error('Permission denied')
    done()
  } else {
    var self = this
...
Helper.prototype.remove = function(apx,req,res,done){
  if(apx.roles && this.options.get('useRoles') && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.remove')){
    res.error('Permission denied')
    done()
  } else {
    var self = this

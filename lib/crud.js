...
Helper.prototype.find = function(apx,req,res,next){
  if(apx.roles && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.find')){
...
Helper.prototype.list = function(apx,req,res,next){
  if(apx.roles && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.list')){
    res.error('Permission denied')
    next()
  } else {
    var data = req.get() || {}
...
Helper.prototype.save = function(apx,req,res,done){
  if(apx.roles && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.save')){
    res.error('Permission denied')
    done()
  } else {
    var self = this
...
Helper.prototype.remove = function(apx,req,res,done){
  if(apx.roles && !apx.roles.getProfile(apx.session.get('profile')).hasRoles(this.name + '.remove')){
    res.error('Permission denied')
    done()
  } else {
    var self = this
